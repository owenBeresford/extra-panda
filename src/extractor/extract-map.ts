...
export class ExtractMap {
  ...
  async compose(base: string, vendor: Array<string>): Promise<HashHashtable> {
    ...
    const BASELINE: Array<string> = this.#reduce.externalFilter(
      this.taggedElements(base),
      vendor,
    );
    ...
    buf = this.#reduce.filterEmpty(
      this.#reduce.filterCommonTags(buf, base, this),
    );
  ...
  taggedElements(base: string): Array<string> {
    ...
    const SELF = this;
    Array.from(this.#dom.querySelectorAll("." + base + " [class]")).map(
      function (a, b) {
        if (a.className.trim() === "") {
          return;
        }
        ...
        list.push(...tmp);
        tmp.map(function (a2, b2) {
          list.push(SELF.treeWalk(a2, a as HTMLElement, base));
        });
      },
    );
 ...
 SEE https://stackoverflow.com/questions/66912181/how-can-i-get-a-css-value-in-its-original-unit-with-javascript
  ...
  extractLocal(sel: string, pseud: Pseudable = null): Hashtable {
    ...
    if (TARGET === null) {
      log(
        ...
        "Value passed '" +
          sel +
          "' into localExtract doesnt work in current doc.",
      );
      return {};
    }
  ...
  treeWalk(cls: string, e: HTMLElement, root: string): string {
    ...
    if (root.includes(" ")) {
      // type changed here
      rootSplit = root.split(" ").map(function (a, b) {
        return a.trim();
      });
    ...
    while (stay) {
      ...
      if (
        rootSplit &&
        e.classList.contains(rootSplit[1]) &&
