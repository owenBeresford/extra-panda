...
const { MorePages, extractTitle, extractAuthor, extractDate, 
		extractDescrip, extractRedirect, mapper } = TEST_ONLY;
...
describe("TEST references MorePages ",  () => {
	it("go 1: MorePages failure ", async () => {
		let tmp=new Promise((good, bad) => {
			const obj2=new PageCollection( [ "https://sample.url/11", "https://sample.url/22" ] );
		...
		.then( (dat)=>{
				assert.equal("HTTP_ERROR, Error: Oh No!", dat.desc, "Error passed through to bad() CB successfully" );
				assert.equal("HTTP_ERROR, Error: Oh No!", dat.title, "Error passed through to bad() CB successfully" );
				// next checkpoint is wrong due to poor input data, but is correct to inputs
				assert.equal("https://google.co.uk/", dat.url, "Error passed through to bad() CB successfully" );
				assert.equal("unknown", dat.auth, "Error passed through to bad() CB successfully" );
					 }, 
				(dat)=>{ 
				assert.equal(1, 0, "K@@@XX0N!!, KL@@@XX0N!! bad, wrong, test die now::"+dat.message );
					 }) ;
	...
	it("go 2: MorePages success ", async () => {
		let tmp=new Promise((good, bad) => {
			const obj2=new PageCollection( ["https://sample.com/11", "https://sample.com/22", "https://sample.com/33"] );
			const obj=new MorePages(obj2, NullVendorMod, 666 );
			obj.promiseExits(good, bad, 0);
			obj.setOffset(0, "https://google.co.uk/");
			let data="<h1>Someting went wrong</h1><br />Please contact the administrat0r. ";
			obj.success("200", data, []);
								})
		.then( 
			(dat)=>{ assert.equal(1, 1, "Passed out right door." ); }, 
			(err)=>{ assert.equal(1, 0, "K@@@XX0N!!, KL@@@XX0N!! bad, wrong, test die now "+err ); }
			);
	...
	it("go 3: MorePages  extractTitle ", () => {
		...
		html="<p>fdhfhfghfghfghfhfg<ul><li>dfgdfgd<li>dfgdgdfgd<li>dgdgdg<li>dgdgdfg</ul><p>dgdgdfgdfgdfgdfg";
		ret=extractTitle(html, "https://sample.url/"); 
	...
	it("go 5: MorePages extractDate ", () => {
		...
		assert.equal(new Date("2025-07-01").getTime(), extractDate({"Last-Modified":"2025-07-01"}, "" ).getTime(), "step1");
		let html="posted   <time datetime=\"1751328000000\">This July</date>";
		assert.equal(new Date("2025-07-01").getTime(), extractDate({}, html ).getTime(), "step2");
	
		html="last updated <time datetime=\"1751328000000\">about now</date>";
		assert.equal(new Date("2025-07-01").getTime(), extractDate({}, html ).getTime(), "step3");

		html="<div class=\"pw-published-date\"><span>1 July 2025 10:00:00</span>";
		assert.equal(new Date("2025-07-01 10:00:00").getTime(), extractDate({}, html ).getTime(), "step4");
...
// extractRedirect( body: string, redirect_limit: number, current: string, loop:number ): Error | boolean
		...
		assert.equal(false, extractRedirect(html, 3, current, 0), "plain webpage, no redirs" );
		assert.equal(false, extractRedirect(html, 3, current, 4), "no idea what should happen here" );

		html="<script> location='https://second.url/page22';";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "plain webpage, first JS redir" );
		assert.equal(false, extractRedirect(html, 3, current, 3), "first JS redir, loop count exeeded" );

		html="<script> location='https://first.url/page11';";
		assert.equal(false, extractRedirect(html, 3, current, 0), "first JS redir to same location" );
		...
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "first JS redir to same location" );
		
		html="<script> location.href='https://second.url/page55';";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "second JS redir" );
	
		html="<script> location.replace('https://second.url/page66');";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "third JS redir" );

		html="<script> location.replaceState(null,\"\",'https://second.url/page77');";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "fourth JS redir" );

		html="<script> location.replaceState({type:\"rabbit\",age:2,name:\"fluffy\"},'','https://second.url/page88');";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "fitth JS redir" );

		html="<link rel=\"canonical\" href=\"https://first.url/page99\" />";
		assert.equal("object", typeof extractRedirect(html, 3, current, 0), "sixth JS redir" );
