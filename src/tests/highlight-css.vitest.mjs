...
describe("TEST highlight CSS", () => {
  it("go 1: CSS test1", async () => {
    const [dom, loc, win, jsdom] = page(
      "http://192.168.0.35/resource/home",
      4,
    );
...
`;
	...
	assert.equal(dom.querySelectorAll('.hljs.language-css').length, 1, "assert #3" );
	assert.equal(dom.querySelectorAll('code[lang="css"]').length, 1, "assert #4" );
	assert.equal(dom.querySelectorAll('code[data-highlighted="yes"]').length, 1, "assert #5" );
});
...
`;
	...
	assert.equal(dom.querySelectorAll('.hljs.language-css').length, 1, "assert #8" );
	assert.equal(dom.querySelectorAll('code[lang="css"]').length, 1, "assert #9" );
	assert.equal(dom.querySelectorAll('code[data-highlighted="yes"]').length, 1, "assert #10" );
});
