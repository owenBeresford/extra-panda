...
`;
    ...
    if (typeof window == "object") {
      assert.equal(dom.querySelectorAll("style").length, 1, "assert #8"); // added by appendAppearance
    }
...
`;
    ...
    if (typeof window == "object") {
      assert.equal(dom.querySelectorAll("style").length, 1, "assert #17"); // added by appendAppearance
    }
