...
export class PageCollection { 
	...
	public get currentBatch():Array<string> {
		const BATCH_NO= Math.ceil(this.list.length /this.batchSz);
console.log("There are "+this.src.length+"/"+this.batchSz+" links in  "+process.argv[3]);
		let ret:Array<string> =[];

		for(let j =0; j< this.batchSz; j++) {
			let offset=this.batchNum*BATCH_SZ +j;
			if(offset >= this.list.length) { break; } // used in last batch, which isn't likely to be full.
