...
import { normaliseString, shorten, publicise_IP, valueOfUrl } from './string-manip';
...
export class MorePages implements HTMLTransformable { 
	...
	public constructor(d:PageCollection, redirect_limit:number=3) {
		...
		log("debug", "Start to annotate "+src.length+" references in this article");
	}
	...
	// I set curl follow-header
		if(( parseInt( statusCode, 10)/100)!==2) {
			console.log("ERROR: "+URL1+"["+this.offset+"] URL was dead "+this.url+" ", statusCode+ " "+ headers.result);
...
console.log("Running cURL close");
		...
		let redir= this.#_extractRedirect(body, this.offset, this.url, this.data.loop );
...
console.log("EXTRACTED {{", item, body, "}}");
	...
	public failure(msg:any):void {
		log("warn", "[url"+this.offset+"] X X X X X X X X X x X X X X X X "+ msg );
		// plan B
	...
	#_extractRedirect(body:string, offset:Readonly<number>, current:string|URL, loop:Readonly<number>):Error|boolean {
