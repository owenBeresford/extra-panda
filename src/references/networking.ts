...
import type { successType, failureType, closeType, HTMLTransformable } = './types';
...
export function fetch2(url:string, good1:successType, bad1:failureType, close:closeType  ):void {
	const curl = new Curl();
	curl.isClose=false;
...
export function exec_reference_url(offset:number, url:string, handler:HTMLTransformable ):Promise<any> {
  return new Promise(async function (good, bad) {	
	...
	try {
		...
		log("warn", "W W W W W W W W W W W W W W W W ["+offset+"] Network error with "+url +" :: "+ e);	
		bad(e);
	}
  })
///////////////////////////////////////////////////////////////////////////////////////////////////////
// sept 2024, this is preferred catch point
	.catch( function(ee) { 
console.warn(ee);
		log("warn", "REDIRECT ["+offset+"] of "+url +" to "+ ee.message);
		if( url!== ee.message) {
