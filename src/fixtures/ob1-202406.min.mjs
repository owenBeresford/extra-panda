...
function B(e2, t2, n2) {
  ...
  }(r2, k, e2, t2, n2), J(e2.querySelector("#popup #sendMasto"), H, e2, t2, n2);
...
let _ = { referencesCache: "/resource/XXX-references", gainingElement: "#biblio", losingElement: ".addReferences", renumber: 1, forceToEnd: 1, maxDescripLen: 230, maxAuthLen: 65, debug: true, runFetch: f };
async function K(e2, t2, r2) {
  if (_ = Object.assign(_, { debug: n(r2) }, e2), 0 === t2.querySelectorAll(a).length)
    return void o("info", "URL '" + r2.pathname + "' isn't marked-up for references, so skipped");
  const i2 = t2.querySelector("#biblio");
  i2 && i2.setAttribute("style", ""), t2.querySelector(_.gainingElement + " *").replaceChildren(), E(_.gainingElement, '<h2 class="biblioSection">References (for mobile UI)</h2> \n<p>The references embedded in the text are displayed here. </p>', t2);
  const l2 = await _.runFetch(m(_.referencesCache, r2), false, r2);
...
`;
      ...
      for (const r3 in e4) {
        ...
        a2 = a2.replace(".", ".  ");
        let i3 = e4[r3].desc + "";
        i3.length > _.maxDescripLen && (i3 = i3.substring(0, _.maxDescripLen));
        let l3 = e4[r3].auth || t3[0];
        "unknown" === e4[r3].auth && (l3 = t3[0]), l3.length > _.maxAuthLen && (l3 = l3.substring(0, _.maxAuthLen)), n2.push({ auth: l3, date: o2, desc: i3, offset: parseInt(r3, 10), title: a2, url: e4[r3].url });
      }
      return n2;
    }(l2.body));
    !function(e4, t3) {
      if (!_.renumber)
        return;
      const n2 = Array.from(t3.querySelectorAll(_.losingElement + " sup a"));
      for (let e5 = 0; e5 < n2.length; e5++)
        n2[e5].textContent = "" + (e5 + 1), _.forceToEnd && (n2[e5].href = "#biblio");
    }(l2.body, t2), E(_.gainingElement, e3, t2);
  } else {
    const e3 = '<p class="error">Unable to get bibliographic data for this article.</p>';
    E(_.gainingElement, e3, t2), o("warn", "Unable to get meta data " + m(_.referencesCache, r2), JSON.stringify(Array.from(l2.headers.entries())));
  }
}
let Y = { indexUpdated: 0, gainingElement: "#biblio", referencesCache: "/resource/XXX-references", renumber: 1, maxAuthLen: 65, debug: true, runFetch: f };
function V(e2) {
...
async function Q(e2, t2, r2, s2) {
  if (Y = Object.assign(Y, { debug: n(r2) }, e2), 0 === t2.querySelectorAll(a).length)
    return void o("info", "This URL '" + r2.pathname + "' isn't marked-up for references, so skipped");
  const l2 = await Y.runFetch(m(Y.referencesCache, r2), false, r2);
  if (l2.ok && Array.isArray(l2.body)) {
    ...
    const n2 = function(e4) {
      ...
      for (let r3 = 0; r3 < e4.length; r3++) {
        ...
        "unknown" === e4[r3].auth && (s3 = t3[0]), s3.length > Y.maxAuthLen && (s3 = s3.substring(0, Y.maxAuthLen)), n3.push("Reference popup for link [" + (r3 + 1) + "]\n\n" + a2 + "\n" + s3 + " " + o2 + "\n\n" + i2);
      }
      return n3;
    }(l2.body);
    !function(e4, t3, n3) {
      ...
      for (let t4 = 0; t4 < e4.length; t4++)
        o2[t4].setAttribute("aria-label", "" + e4[t4]), z(o2[t4], n3), Y.renumber && (o2[t4].textContent = "" + r3), r3++;
      if (o2.length > e4.length) {
        t3.querySelector("p[role=status]").textContent += "Recompile meta data";
        let r4 = e4.length;
        for (; r4 < o2.length; ) {
          const e5 = V(r4);
          o2[r4].setAttribute("aria-label", "" + e5), z(o2[r4], n3), Y.renumber && (o2[r4].textContent = "" + (r4 + 1)), r4++;
        }
      }
...
`;
    ...
    const e3 = '<p class="error">Unable to get bibliographic data for this article.</p>';
    E(Y.gainingElement, e3, t2), o("warn", "Unable to get meta data " + m(Y.referencesCache, r2), JSON.stringify(Array.from(l2.headers.entries())));
  }
}
function Z(e2, t2, n2) {
...
function ae() {
  return re.pageInitRun;
}
!async function(e2, t2, r2, a2) {
  re = Object.assign(re, {}, e2);
  const i2 = n(r2);
  if (re.pageInitRun)
  ...
  !function(e3, t3) {
    ...
    for (let e4 = 0; e4 < n2.length; e4++)
      n2[e4].setAttribute("style", "--offset-height: " + q(n2[e4], t3)[0] + "px;");
  ...
  Z(c2, t2, a2), function(e3, t3, n2) {
    t3.querySelectorAll("article a").forEach(function(r3) {
      "docs" === p(r3).trim().toLowerCase() && (r3.textContent = "", E(r3, '<i class="fa fa-book-open" aria-hidden="true"></i>\n		 <span class="sr-only">docs</span>', t3), r3.setAttribute(e3 ? "aria-label" : "title", "Link to the project docs; it may be a git page, or a separate webpage. "), e3 && z(r3, n2));
    });
  ...
  }(t2), ne(t2), T(1040, t2, r2, a2), !x(t2, r2, a2) && "/resource/home" !== r2.pathname && t2.querySelectorAll(".reading").length < 2 && function(e3, t3, r3) {
    const a3 = Object.assign({}, { timeFormat: "m", dataLocation: ".blocker", target: "#shareGroup", wordPerMin: 275, codeSelector: "code", refresh: false, debug: n(r3) }, e3), i3 = a3.dataLocation + " img, " + a3.dataLocation + " picture, " + a3.dataLocation + " object", s3 = ee(a3.dataLocation, t3);
    if (!s3)
      return;
    let l2 = 0;
    a3.codeSelector && (l2 += ee(a3.codeSelector, t3));
    ...
    c3 = Math.round(c3);
    const u3 = '<a class="reading" title="See longer version of this reading guide." href="/resource/jQuery-reading-duration">To read: ' + c3 + a3.timeFormat + "</a>";
    E(a3.target, u3, t3);
  }({ dataLocation: "#main", target: ".addReading", debug: i2, refresh: true }, t2, r2);
  {
    const e3 = t2.querySelectorAll(".tabComponent");
    for (let n2 = 0; n2 < e3.length; n2++) {
      const r3 = Array.from(e3[n2].querySelectorAll(".tab-title a"));
      for (let e4 = 0; e4 < r3.length; e4++)
        u2 = r3[e4], d2 = (e5) => {
          oe(e5, t2);
        }, u2.addEventListener("click", d2), u2.addEventListener("touch", d2), u2.addEventListener("keypress", d2);
    }
    "" !== r2.hash && oe(r2.hash, t2);
  }
  var u2, d2;
  if (r2.pathname.match("group-")) {
    ...
    e3 && await D({ group: e3, debug: i2, runFetch: "adjacentRunFetch" in re ? re.adjacentRunFetch : f }, t2, r2, a2);
  } else {
    x(t2, r2, a2) ? await K({ debug: i2, renumber: 1, runFetch: "mobileRunFetch" in re ? re.mobileRunFetch : f }, t2, r2) : await Q({ debug: i2, renumber: 1, runFetch: "desktopRunFetch" in re ? re.desktopRunFetch : f }, t2, r2, a2);
    ...
    if (0 === e3.length)
      ...
      for (let n2 = 0; n2 < e3.length; n2++)
        await D({ group: e3[n2], debug: i2, iteration: n2, count: e3.length, runFetch: "adjacentRunFetch" in re ? re.adjacentRunFetch : f }, t2, r2, a2);
  }
