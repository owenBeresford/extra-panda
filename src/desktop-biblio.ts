...
// * @protected
let OPTS:BiblioProps ={} as BiblioProps; 
 ...
 * @return {void}
 */
function markAllLinksUnknown(dom:Document=document, loc:Location=location):void {
	const naam:string= articleName( loc); 
	const WASSUP:Array<HTMLAnchorElement> =dom.querySelectorAll( ALL_REFERENCE_LINKS);
...
function addMetaAge(xhr:SimpleResponse, dom:Document=document) {
	let updated:number =( new Date(xhr.headers.get('last-modified')) ).getTime();
	if( updated >0 ) {
		let str:string="<span>Links updated <time datetime=\""+updated+"\" title=\"When this was last recompiled\">"+((new Date(updated)).toLocaleDateString('en-GB', {hour12:false, dateStyle:"medium" })) +"</time> </span>";
 ...
 * @return {void}
 */
async function createBiblio(opts:BiblioProps, dom:Document=document, loc:Location=location) {
	OPTS = Object.assign( {
		indexUpdated:0,
		type:'biblio',
		width:400,
	...
	const ROOT=access();
	let data:SimpleResponse=await ROOT.runFetch(makeRefUrl(OPTS.referencesCache, loc), false); 
	if(!data.ok || !Array.isArray(data.body) ) {
		markAllLinksUnknown(dom, loc);
		const html="<p class=\"error\">Unable to get bibliographic data for this article.</p>";
...
//		throw new Error("ERROR getting references "+makeRefUrl(OPTS.referencesCache, loc));
	...
	addMetaAge(data);
	let cooked:Array<string>= normaliseData(data.body as Array<ReferenceType>);
